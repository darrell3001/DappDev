...
function initContract() {
	...
	contract.events.counterUpdated((error, event) => { 
		if (!error) {
			...
			if (error.message.includes("The current provider doesn't support subscriptions")) {
				$('#errormsg').html("timer pop");
				setInterval(function(){ timerPop(); }, 5000);
			} else {
...
window.addEventListener('load', async () => {
	...
	// Maybe older version of Metamask
		...
		$('#errormsg').html("Legacy version of digital wallet detected. Please consider upgradging to a more recent version of Metamask");
		initContract();
		getValue();
	}
		// Non-dapp browsers...
			...
			$('#errormsg').html("Non-Ethereum browser detected. You should consider trying MetaMask!");
		}
	}
})
...
$('#increment').click(function() {
	contract.methods.increment().send({from: fromAddress}, function(err, tx) {
...
$('#decrement').click(function() {
	contract.methods.decrement().send({from: fromAddress}, function(err, tx) {
		$('#status').html("waiting for confirmation");
		$('#last-transaction-status').html("transaction is pending");
	})
	.on('confirmation', (function() {
...
$('#reset').click(function() {
	contract.methods.reset().send({from: fromAddress}, function(err, tx) {
		$('#status').html("waiting for confirmation");
		$('#last-transaction-status').html("transaction is pending");
	})
	.on('confirmation', (function() {
